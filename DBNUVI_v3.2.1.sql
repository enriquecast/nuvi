...
estado enum('Abierto','En asignación','En proceso','cerrado')
);
...
sueldo INT,
disponibilidad enum('Disponible','Asignado a proyecto'),
fkProyecto INT,
...
CREATE TABLE CASOS(
    ...
    DetalleProblema varchar(300),
    DetalleSolución varchar(300),
    EstadoCaso enum('Por asignación','Asignado','Resuelto','Cerrado'),
    fkUsuario int,
    foreign key (fkUsuario) references USUARIOS(idUsuario),
...
/*Consulta reportes*/    
    SELECT tipoproblematicas.tipoProblematica, COUNT(*) as Cantidad
...
SELECT count(fechaVisita) 
FROM familias where fechaVisita = (date_format(fechaVisita, '%m') = '03');


SELECT if (date_format(fechaVisita, '%m') = '03', '','Otro mes' ) from familias;
	
SELECT tipoproblematicas.tipoProblematica, COUNT(*)
FROM tipoproblematicas 
GROUP BY tipoproblematica;
